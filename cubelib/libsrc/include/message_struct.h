...
typedef struct tagMessage_Expand_Data  //general expand data struct
{
   ...
   BYTE data[0];
} __attribute__((packed)) MESSAGE_EXPAND;

struct expand_extra_info  //expand data struct to store one or more DIGEST_SIZE*2 size info
{
   int  data_size;   //this expand data's size
   char tag[4];      //expand data's type, should be "EEIE"
   BYTE  uuid[DIGEST_SIZE*2];
} __attribute__((packed)) MESSAGE_EXPAND_EXTRAINFO;

struct Expand_Data_State   //
{
	int  data_size;		// This should be equal to 8
	char tag[4];    // This should be "STAE"
	BYTE nonce[DIGEST_SIZE];
	int  state_machine_no; // which state machine it belongs to
	int  state;            // this state machine's state 
};

enum  identify_type
{
	NO_IDENTIFY,
	IDENTIFY_WITH_PUBKEY,
	IDENTIFY_WITH_CACERT
};

struct expand_data_identify
{
	int  data_size;              // this should be equal to DIGEST_SIZE*4+12+strlen(user_name)+1+blob
	char tag[4];                 // this should be "IDEE"
	BYTE nonce[DIGEST_SIZE];
...
struct expand_data_forward
{
	int  data_size;             
	char tag[4];                 // this should be "FORE"
	BYTE nonce[DIGEST_SIZE];
