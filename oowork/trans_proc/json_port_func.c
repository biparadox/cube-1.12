...
int json_port_start(void * sub_proc,void * para)
{
	...
	while(sec_subject_recvmsg(sub_proc,&message_box)>=0)
	{
		...
		channel_conn = hub_get_first_connector(port_hub);
		while(channel_conn!=NULL)
		{
	  		if(connector_get_type(channel_conn)==CONN_CHANNEL)
			{
                		channel_conn->conn_ops->write(channel_conn,
                        	       buffer,stroffset);
			...
			channel_conn = hub_get_next_connector(port_hub);
		}	
	}
